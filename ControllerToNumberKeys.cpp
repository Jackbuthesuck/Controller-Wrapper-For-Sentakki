...
private:
    ...
    // No touch control in this version
    int overlayPosX;  // Store overlay position for visualization
    int overlayPosY;
...
private:
    ...
    void handleRadialMenu(int direction, bool& haveKey, std::string& currentKey, const std::string& otherKey) {
        std::string newKey = std::to_string(direction + 1);
        
        // Check if the other stick is already using this key
        if (newKey == otherKey) {
            // Skip - the other stick is already pressing this key
            if (haveKey) {
                // Release current key if we have one
                simulateKeyPress(currentKey, false);
                haveKey = false;
                currentKey = "null";
                std::cout << "Direction conflicts with other stick, releasing key" << std::endl;
            }
            return;
        }
        ...
        if (haveKey) {
            ...
            if (currentKey != newKey) {
                ...
                std::cout << "Direction changed: switching to key " << currentKey << std::endl;
            }
            // If same direction, keep the key held (do nothing)
        } else {
            // First press
            haveKey = true;
            currentKey = newKey;
    ...
    void leftRadialMenuHandler(int direction) {
        handleRadialMenu(direction, lHaveKey, lCurrentKey, rCurrentKey);
    }

    void rightRadialMenuHandler(int direction) {
        handleRadialMenu(direction, rHaveKey, rCurrentKey, lCurrentKey);
    }
    ...
    void updateDebugInfo(double lAngle, double rAngle, int lDirection, int rDirection, const DIJOYSTATE* diState = nullptr) {
        ...
        std::string info = "=== CONTROLLER TO NUMBER KEYS ===\r\n";
        info += "Version: Number Keys Only\r\n";
    ...
    void run() {
        ...
        while (true) {
            ...
            if (controllerSuccess) {
                ...
                // Handle button presses for number keys
...
int main() {
    ...
    std::cout << "=== CONTROLLER TO NUMBER KEYS ===" << std::endl;
    std::cout << "Version: Number Keys Only" << std::endl;
    std::cout << "Bumpers trigger number keys 1-8 based on stick direction" << std::endl;
